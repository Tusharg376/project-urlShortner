...
const createUrl = async function (req, res) {
    ...
    try {
        ...
        let data1 = JSON.parse(alreadyCreated);
        ...
        if(uniqueUrl){
            await SETEX_ASYNC(data.longUrl,30,JSON.stringify(uniqueUrl));
        ...
        await SETEX_ASYNC(`${data.longUrl}`,30, JSON.stringify({ status: true, data: { urlCode, longUrl, shortUrl } }))
...
const getUrl = async function (req, res) {
    ...
    try {
        ...
        await SETEX_ASYNC(`${url}`,30, JSON.stringify(longUrl.longUrl));
